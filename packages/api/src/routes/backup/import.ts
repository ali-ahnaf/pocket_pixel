...
import Joi from "joi";
import { BackupDebtDto, BackupRecurringSkipDto, BackupTransactionDto, ImportDataRequest, RecurringDto, TagDto, User, UserPreferenceDto, VaultDto } from "@expense-tracker/shared";
...
const importDataSchema = Joi.object<ImportDataRequest>({
  ...
  data: Joi.object({
    ...
    debts: Joi.array().items(Joi.object<BackupDebtDto>({
      id: Joi.string().uuid().required(),
      userId: Joi.string().uuid().required(),
    ...
    vaults: Joi.array().items(Joi.object<VaultDto>({
      id: Joi.string().uuid().required(),
      userId: Joi.string().uuid().required(),
      name: Joi.string().max(100).required(),
    ...
    transactions: Joi.array().items(Joi.object<BackupTransactionDto>({
      id: Joi.string().uuid().required(),
      userId: Joi.string().uuid().required(),
    ...
    recurrings: Joi.array().items(Joi.object<RecurringDto>({
      id: Joi.string().uuid().required(),
      userId: Joi.string().uuid().required(),
      title: Joi.string().max(200).allow(null, '').required(),
      amount: Joi.number().positive().precision(2).required(),
      type: Joi.string().valid('expense', 'income').required(),
      date: Joi.string().pattern(/^\d{4}-\d{2}-\d{2}$/).allow(null).required(),
    ...
    recurringSkips: Joi.array().items(Joi.object<BackupRecurringSkipDto>({
