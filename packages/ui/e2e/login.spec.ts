...
test.describe('Sign-in page', () => {
  ...
  test('signs in successfully and redirects to the dashboard', async ({ page }) => {
    ...
    const token = await page.evaluate(() => window.localStorage.getItem('pocket_pixel_profile'));
    expect(token).toBe(JSON.stringify({ id: AUTH_RESPONSE.id, name: AUTH_RESPONSE.name, email: AUTH_RESPONSE.email, avatar: AUTH_RESPONSE.avatar }));
  });
